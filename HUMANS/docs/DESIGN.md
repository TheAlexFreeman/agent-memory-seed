...
- *On-demand:* `session-checklists.md`, `curation-policy.md`, `update-guidelines.md` (loaded on full bootstrap), `curation-algorithms.md` (loaded during aggregation or stage transitions), `update-guidelines.md` § "Worked example" (reference for first read-only session), `system-maturity.md` (loaded during periodic review).
