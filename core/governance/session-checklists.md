...
1. Follow the compact returning manifest in `meta/quick-reference.md`: orient with non-placeholder `projects/SUMMARY.md` first, then load `identity/SUMMARY.md`, non-placeholder `chats/SUMMARY.md`, and substantive scratchpad files. Load `plans/SUMMARY.md`, `knowledge/SUMMARY.md`, or `skills/SUMMARY.md` only when the active project, recent history, or current task makes them relevant.
