...
| **Compact returning** | this file → `memory/working/projects/SUMMARY.md` _(skip if empty or still placeholder)_ → `memory/users/SUMMARY.md` → `memory/activity/SUMMARY.md` _(skip if empty or still placeholder)_ → `memory/working/scratchpad/USER.md` _(skip if only placeholder)_ → `memory/working/scratchpad/CURRENT.md` _(skip if only placeholder)_ → task-relevant `memory/working/projects/SUMMARY.md` plus task-relevant `memory/knowledge/SUMMARY.md` and/or `memory/skills/SUMMARY.md` only when the active project, recent history, or current task makes them relevant |
