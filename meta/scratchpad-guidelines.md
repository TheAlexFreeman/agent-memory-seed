...
> **Load:** On-demand — when you need to write to `scratchpad/` or when reviewing scratchpad lifecycle during session end. The compact read/write steps for normal sessions are in `meta/session-checklists.md`. You do not need to load this file just to read `scratchpad/USER.md` or `scratchpad/CURRENT.md`.

The `scratchpad/` folder is a staging area that sits between ephemeral session context (which disappears) and formal memory (which requires governance overhead). It operates below the governance layer: no frontmatter, no approval requirements, no CHANGELOG entries. Content here is either promoted to formal memory when it earns it, or cleared when it doesn't.
...
`scratchpad/USER.md` belongs to the user. It is their channel to the agent — a place to inject current priorities, temporary constraints, and session context without going through the formal memory system.
...
`scratchpad/CURRENT.md` is the agent's working notes file — a place to track provisional observations, draft abstractions, and cross-session hypotheses that aren't yet ready for formal memory.
...
| Draft abstraction awaiting more evidence | `scratchpad/CURRENT.md` or a dated file in `scratchpad/` |
...
- **Date the entry** and link the session: `<!-- YYYY-MM-DD, session: chats/YYYY/MM/DD/chat-NNN -->`
...
For longer working content — draft knowledge files, partial skill definitions, extended hypotheses — create a separate dated file: `scratchpad/YYYY-MM-DD-brief-topic.md`. Reference it from CURRENT.md so it's discoverable. CURRENT.md is the index; dated files hold the detail.
...
Dated working files (`scratchpad/YYYY-MM-DD-topic.md`) follow the same logic. During aggregation, check whether any dated files older than the active low-trust retirement threshold are no longer referenced from CURRENT.md — if so, move them to `scratchpad/_archive/`.
...
- **Instructions or procedures** — instruction containment applies: only `skills/` and `meta/` files may instruct. "Next time, do X" is a skill proposal candidate, not a scratchpad entry.
