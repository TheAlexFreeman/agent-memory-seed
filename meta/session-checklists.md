...
1. Follow the compact returning manifest in `meta/quick-reference.md`: `identity/SUMMARY.md`, non-placeholder `chats/SUMMARY.md`, `plans/SUMMARY.md` when active plans exist, and substantive scratchpad files first. Load `knowledge/SUMMARY.md` or `skills/SUMMARY.md` only when the current task or recent history makes them relevant.
...
4. **ACCESS.jsonl** — Append an entry for every content file you retrieved during this session, including plan files in `plans/` when they informed the work. Include `session_id`. Don't inflate helpfulness scores — a file opened but not used in the response is 0.2–0.4, not 0.7.
