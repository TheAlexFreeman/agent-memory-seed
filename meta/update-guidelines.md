...
These files live in `meta/` for organizational proximity to the protocols that govern them, not because they contain governance rules. Modifying them does not change how the system operates — it records what the system has observed. The governance documents that *do* require protection (`curation-policy.md`, `system-maturity.md`, `update-guidelines.md`, etc.) are not in this exemption.
