...
- **[zfc-set-theory.md](knowledge/_unverified/mathematics/logic-foundations/zfc-set-theory.md)** — ZFC set theory — historical crisis (Russell's paradox), all 10 axioms with formal statements (extensionality, empty set, pairing, union, power set, infinity, separation, replacement, foundation, choice), axiom of choice (6 equivalent formulations, independence via Gödel's L and Cohen's forcing, Banach-Tarski), continuum hypothesis (independence, contemporary perspectives: multiverse view, Ultimate-L, forcing axioms), cumulative hierarchy V, inner models, large cardinal hierarchy (inaccessible through rank-into-rank), what ZFC can and cannot resolve, and criticisms (constructivism, predicativism, structuralism).
- **[category-theory-foundations.md](knowledge/_unverified/mathematics/logic-foundations/category-theory-foundations.md)** — Category theory as alternative foundation — basic definitions (categories, functors, natural transformations), universal properties and the Yoneda lemma (objects determined by relationships), adjunctions and monads (free-forgetful, product-exponential, monadic programming), topos theory (elementary toposes, subobject classifier, internal logic), ETCS as structural alternative to ZFC (equiconsistent, structural vs material identity), univalent foundations/HoTT (univalence axiom: equivalence = identity), categorical semantics of programming languages, and applied category theory.
