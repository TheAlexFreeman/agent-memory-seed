...
7. If write access is unavailable, do not attempt the write. Instead, produce the confirmed profile using the **onboarding export format** (see `scripts/onboard-export-template.md`): output a single markdown document with top-level YAML frontmatter for `session_id` and `session_date`, followed by `## Identity Profile`, `## Session Transcript`, `## Session Summary`, and `## Session Reflection` sections. Use the canonical session path form (`chats/YYYY/MM/DD/chat-001`) for `session_id`. Tell the user to save this output to a file and run `bash scripts/onboard-export.sh <file>` to import it into the repo. This replaces the generic deferred-action format for onboarding specifically, since the export script preserves the first session's metadata, transcript, and chat artifacts while handling the repo writes and commit.
